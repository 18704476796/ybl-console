...
export default {
  ...
  methods: {
    ...
    edit (locked, userName, realName, phoneNum, roleName, email, id) { // 编辑用户弹层函数
      console.log(email)
      this.formEdit.locked = locked
      this.formEdit.userName = userName
      this.formEdit.realName = realName
      this.formEdit.roleName = roleName
  ...
  data () {
    ...
    return {
      ...
      rules: { // 表单验证.
        ...
        roleUser: [{ required: true, message: '请输入姓名', trigger: 'blur' }],
        roleTel: [{ required: true,validator: checkPhone, trigger: 'blur' }],
        email: [
